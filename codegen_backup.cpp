...
Function *MethodDecl::codegen() {
    ...
    if (*(return_type) == "void") current_function_ret_bits = 0;
    if (*(return_type) == "int") current_function_ret_bits = 32;
    if (*(return_type) == "boolean") current_function_ret_bits = 1;
...
Value *AssignStmt::codegen() {
    ...
    if (right_part->get_expr_type() == "MethodCall") {
        // Ensure the corresponding method returns something (int / bool)
        Function * to_load_function = LLVMmodule->getFunction(*(((MethodCall *) right_part)->method_name->variable_name));
        if (to_load_function->getReturnType()->getPrimitiveSizeInBits() == 0) {
            logger_class->add("Function return type should not be void", this->lineno);
        }
    }
    ...
    if(*op == "="){
        ...
        if (l == r){
            ...
            logger_class->add("Invalid Operand for Assignment operation: '"+ *op + "', lhs and rhs must have same type.", this->lineno);
            return nullptr;
        }
    }
    ...
    if(*op == "+=" || *op == "-="){
        ...
        if (l == r && l == temp->getType()){
            ...
            logger_class->add("Invalid Operand for Assignment operation: '"+ *op + "', rhs must have 'int' type.", this->lineno);
            return nullptr;
        }
    }
...
Value *Location::codegen() {
    ...
    if (array_list.find(*(var_name->variable_name)) == array_list.end()) {
        logger_class->add("'" + (*(var_name->variable_name)) + "' is not an array variable", this->lineno);
        if (debug == true) {
            call_stack_size --;
            cout << debug_info() << "Out codegen of: Location" << endl << flush;
        }
    ...
    if (final_ans != 32) {
        logger_class->add("Array index should be an integer", this->lineno);
    }
    
    if (index->get_expr_type() == "MethodCall") {
        // Ensure the corresponding method returns something (int / bool)
        Function * to_load_function = LLVMmodule->getFunction(*(((MethodCall *) index)->method_name->variable_name));
        if (to_load_function->getReturnType()->getPrimitiveSizeInBits() == 0) {
            logger_class->add("Function return type should not be void", this->lineno);
        }
    }
...
Value *IfElseStmt::codegen() {
    ...
    bool break_if = if_block->has_break(), break_else = else_block->has_break();
    ...
    if (!ret_if && !break_if && !if_block->has_continue()) {
    ...
    if (else_block != nullptr) {
        ...
        if (!ret_else && !break_else && !else_block->has_continue()) {
            MagicalBuilder.CreateBr(nextBlock);
        }
    }
...
Value *IfStmt::codegen() {
    ...
    if (final_ans != 1) {
        logger_class->add("Condition inside an if-else statement must be a boolean.", this->lineno);
        return nullptr;
    }
    ...
    bool break_if = if_block->has_break();
...
Value *ForStmt::codegen() {
    ...
    if (start == nullptr) {
        ...
        logger_class->add("For loop start condition error.", this->lineno);
        return nullptr;
    }
    ...
    if (final_ans != 32) {
        logger_class->add("Loop start condition should be of type 'int'.", this->lineno);
        return nullptr;
    }

    if (start_cond->get_expr_type() == "MethodCall") {
        // Ensure the corresponding method returns something (int / bool)
        Function * to_load_function = LLVMmodule->getFunction(*(((MethodCall *) start_cond)->method_name->variable_name));
        if (to_load_function->getReturnType()->getPrimitiveSizeInBits() == 0) {
            logger_class->add("Function return type should not be void", this->lineno);
            return nullptr;
        }
    ...
    if (final_ans != 32) {
        logger_class->add("Loop end condition should be of type 'int'.", this->lineno);
        return nullptr;
    }

    if (right_cond->get_expr_type() == "MethodCall") {
        // Ensure the corresponding method returns something (int / bool)
        Function * to_load_function = LLVMmodule->getFunction(*(((MethodCall *) right_cond)->method_name->variable_name));
        if (to_load_function->getReturnType()->getPrimitiveSizeInBits() == 0) {
            logger_class->add("Function return type should not be void", this->lineno);
        }
    }
    ...
    if (!code_block->has_break() && !code_block->has_continue()) {
        Value *cur = Variable;
...
Value *RetExpr::codegen() {
    ...
    if (current_function_ret_bits != final_ans) {
        logger_class->add("Return statement doesn't match function's return type.", this->lineno);
    ...
    if (expr->get_expr_type() == "MethodCall") {
        // Ensure the corresponding method returns something (int / bool)
        Function * to_load_function = LLVMmodule->getFunction(*(((MethodCall *) expr)->method_name->variable_name));
        if (to_load_function->getReturnType()->getPrimitiveSizeInBits() == 0) {
            logger_class->add("Function return type should not be void", this->lineno);
        }
    }
...
Value *BinaryOpExpression::codegen() {
    ...
    if (*op == "+" || *op == "-" || *op == "*" || *op == "/" || *op == "%" ||*op == "<" || *op == ">" || *op == ">=" || *op == "<=") {
        ...
        if (l != temp->getType() || r != temp->getType()){
            logger_class->add("Invalid Operand for binary operation: '"+ *op + "' , should be int", this->lineno);
            return nullptr;
        }
    }
    ...
    if (*op == "==" || *op == "!=") {
        ...
        if (l == r && (l==temp1->getType() || r==temp->getType())) {
            ...
            logger_class->add("Invalid Operand for Equal operation: '"+ *op + "' , should be int or boolean", this->lineno);
            return nullptr;
        }
    }
    ...
    if(*op == "&&" || *op == "||"){
        ...
        if (l == r && l==temp1->getType()) {
            ...
            logger_class->add("Invalid Operand for Conditional operation: '"+ *op + "', should be boolean", this->lineno);
            return nullptr;
        }
    }

    if (left->get_expr_type() == "MethodCall") {
        // Ensure the corresponding method returns something (int / bool)
    ...
    if (right->get_expr_type() == "MethodCall") {
        // Ensure the corresponding method returns something (int / bool)
...
Value *MethodCall::codegen() {
    ...
    for (auto &arg : args_list) {
        ...
        if (final_ans != arg_rn->getType()->getPrimitiveSizeInBits()) {
            logger_class->add("Method call doesn't match the function signature.", this->lineno);
            if (debug == true) {
                call_stack_size --;
                cout << debug_info() << "Out codegen of: MethodCall" << endl << flush;
            }
...
Value *CalloutArg::codegen() {
    ...
    if (arg_expr->get_expr_type() == "MethodCall") {
        // Ensure the corresponding method returns something (int / bool)
        Function * to_load_function = LLVMmodule->getFunction(*(((MethodCall *) arg_expr)->method_name->variable_name));
        if (to_load_function->getReturnType()->getPrimitiveSizeInBits() == 0) {
            logger_class->add("Function return type should not be void", this->lineno);
        }
    }
